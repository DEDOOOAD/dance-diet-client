...
const DEFAULT_LIVE_SESSION_UUID = 'user-lee';
...
export function useDanceVisionPracticeModalController({
  ...
  const liveSessionUuid = useMemo(() => getLiveSessionUuid(), []);
  ...
  const handleServerMessage = useCallback(
    (event: MessageEvent) => {
      try {
        ...
        if (payload.type === 'frame_result' || payload.type === 'frame_ack') {
  ...
  const createLiveSession = useCallback(
    ...
    [clearCaptureLoop, handleServerMessage, liveSessionUuid, logDebug, updateBufferedMetrics, updateBufferedTransportStats]
  );
  ...
  }, [captureVisionFrame, clearCaptureLoop, liveSessionUuid, liveUserWeightKg, logDebug, updateBufferedTransportStats]);
