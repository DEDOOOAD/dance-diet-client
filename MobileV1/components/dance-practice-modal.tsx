...
function getHttpBaseUrl() {
  return `http://${getDevServerHost()}:${SERVER_PORT}`;
}
...
function normalizeWsUrl(wsUrl: string | undefined, sessionId: string) {
  if (!wsUrl) {
    return `ws://${getDevServerHost()}:${SERVER_PORT}/ws/live/${sessionId}`;
  }

  const fallbackHttpOrigin = getHttpBaseUrl();
  ...
  try {
    ...
    if (
      ...
      parsedUrl.hostname = getDevServerHost();
    }
    return parsedUrl.toString();
  } catch {
    return `${fallbackHttpOrigin.replace(/^http/, 'ws')}/ws/live/${sessionId}`;
  }
}
...
export function DancePracticeModal({ dance, visible, onClose }: DancePracticeModalProps) {
  ...
  const createLiveSession = useCallback(
    ...
    [clearTimeout, handleServerMessage, logDebug]
  );
        ...
        {cameraPermission?.granted && (
          <View style={styles.hiddenCameraWrap} pointerEvents="none">
            <CameraView
              ...
              onMountError={(event) => {
                logDebug('Camera mount error', { message: event.nativeEvent.message });
                setStreamState('error');
              }}
              onCameraReady={() => {
