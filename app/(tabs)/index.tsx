...
const EMPTY_DAY_METRICS: DayMetrics = {
  burnedCalories: 0,
  burnedGoal: 0,
  workoutMinutes: 0,
...
export default function HomeScreen() {
  ...
  const weekDatesKey = useMemo(() => baseWeekDays.map((day) => day.dateKey).join('|'), [baseWeekDays]);
  const weekDates = useMemo(() => baseWeekDays.map((day) => day.date), [weekDatesKey]);
  ...
  }, [currentUserId, weekDates, weekDatesKey]);
