...
import { clearCurrentUserUuid } from '@/services/current-user';
...
const MENU_SECTIONS: Array<{ title: string; items: MenuItem[] }> = [
  ...
  },
  {
    ...
    items: [
      ...
      { icon: 'musical-notes-outline', label: '배경 음악 자동 재생', toggle: true, defaultOn: true },
    ],
  },
  {
    title: '기타',
    items: [
      ...
      { icon: 'log-out-outline', label: '로그아웃', variant: 'destructive', key: 'logout' },
    ],
  },
];
...
function MenuRow({ item, onPress }: { item: MenuItem; onPress?: () => void }) {
  ...
  const iconColor = item.variant === 'destructive' ? colors.accent : colors.text2;
  ...
  const content = (
    <>
      ...
      <Text style={[styles.menuLabel, item.variant === 'destructive' && styles.menuLabelDestructive]}>
        {item.label}
      </Text>
      ...
      {item.toggle && (
        <Switch
          ...
          thumbColor={enabled ? colors.accent : colors.text3}
            ...
            <View style={styles.bodyCardHeader}>
              ...
              <View style={styles.bodyCardText}>
                ...
                <Text style={styles.bodyCardSub}>
                  키, 체중, 목표 체중과 기간을 입력하면 루틴 추천과 칼로리 가이드에 활용할 수 있어요.
                </Text>
              </View>
            </View>
          ...
          <View key={section.title} style={styles.section}>
            ...
            <View style={styles.menuGroup}>
              {section.items.map((item, index) => (
                <View key={item.label}>
                  <MenuRow item={item} onPress={item.key ? () => handleMenuPress(item) : undefined} />
          ...
          <Pressable style={styles.modalCard} onPress={(event) => event.stopPropagation()}>
            <View style={styles.modalHeader}>
              <View>
                ...
                <Text style={styles.modalSub}>프로필에 사용할 기본 정보를 입력해 주세요.</Text>
              </View>
