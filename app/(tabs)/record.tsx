...
import {
  fetchMonthlyRecords,
  fetchWeightRecords,
  type RecordMonthSession,
  type RecordWeightResponse,
  type WeightHistoryPoint,
} from '@/services/record-tab';
...
function mapMonthlySessionsToDisplay(sessions: RecordMonthSession[]): Session[] {
  return sessions.map((item, index) => {
    const fallbackDate = new Date(`${item.dateKey}T00:00:00`);
    const parsedDate = item.performedAt ? new Date(item.performedAt) : fallbackDate;
    ...
    return {
      ...
      weekday: WEEKDAYS[date.getDay()] ?? '-',
      time: formatRecordTime(item.performedAt),
      name: item.danceName,
      kcal: item.kcal,
...
function mapWeightHistoryToDisplay(history: WeightHistoryPoint[], selectedMonth: Date): WeightPoint[] {
  const sorted = history
    .map((item) => {
      ...
      return {
        ...
        delta: 0,
        note: item.note,
        active: sameMonth(date, selectedMonth),
  ...
  return sorted.map((item, index) => ({
    ...item,
    delta: index === 0 ? 0 : Number((item.kg - sorted[index - 1].kg).toFixed(1)),
  }));
}
...
export default function RecordScreen() {
  ...
  const [userId, setUserId] = useState(() => getCurrentUserUuid());
  ...
  const [monthlyData, setMonthlyData] = useState<Awaited<ReturnType<typeof fetchMonthlyRecords>> | null>(null);
  const [weightData, setWeightData] = useState<RecordWeightResponse | null>(null);
  ...
  useEffect(() => {
    ...
    void (async () => {
      const nextUserId = await loadCurrentUserUuid();
      console.log('[record] loaded current user uuid', nextUserId);
      if (!cancelled) {
  ...
  useEffect(() => {
    ...
    void (async () => {
      console.log('[record] fetching record data', {
        userId,
        selectedMonth: selectedMonth.toISOString(),
      });
      try {
        const [nextMonthlyData, nextWeightData] = await Promise.all([
          fetchMonthlyRecords(userId, selectedMonth),
          fetchWeightRecords(userId, selectedMonth),
        ]);
        ...
        console.log('[record] fetched record data', {
          userId,
          monthlySessionCount: nextMonthlyData.summary.sessionCount,
          monthlyActiveDays: nextMonthlyData.summary.activeDays,
          weightHistoryCount: nextWeightData.history.length,
        });
      } catch (error) {
        if (cancelled) {
          return;
        ...
        setMonthlyData(null);
        setWeightData(null);
        setRecordError(error instanceof Error ? error.message : '기록 데이터를 불러오지 못했어요.');
      } finally {
        if (!cancelled) {
          setIsLoading(false);
        }
  ...
  const sessions = useMemo(() => mapMonthlySessionsToDisplay(monthlyData?.sessions ?? []), [monthlyData]);
  ...
  const monthSummary = useMemo(
    () => ({
      kcal: monthlyData?.summary.totalKcal ?? 0,
      minutes: monthlyData?.summary.totalMinutes ?? 0,
      activeDays: monthlyData?.summary.activeDays ?? 0,
    }),
    [monthlyData]
  );
  const monthlyStats = useMemo(() => {
    const sessionCount = monthlyData?.summary.sessionCount ?? 0;
    const avgKcalPerSession = monthlyData?.summary.avgKcalPerSession ?? 0;
    const peak = monthlyPoints.reduce(
      (best, point) => (point.kcal > best.kcal ? point : best),
      monthlyPoints[0] ?? { label: '-', kcal: 0 }
    );

    return [
      { label: '이번 달 소모', value: monthSummary.kcal.toLocaleString(), unit: 'kcal', icon: 'flame', color: colors.accent },
      { label: '총 운동 시간', value: durationText(monthSummary.minutes), icon: 'time', color: colors.purple },
      { label: '운동한 날짜', value: String(monthSummary.activeDays), unit: '일', icon: 'calendar', color: colors.teal },
      { label: '주간 피크', value: peak.label, unit: `${peak.kcal.toLocaleString()} kcal`, icon: 'sparkles', color: colors.accent2 },
      { label: '세션 평균 소모', value: avgKcalPerSession.toLocaleString(), unit: 'kcal', icon: 'pulse', color: '#22C55E' },
      { label: '총 세션 수', value: String(sessionCount), unit: '회', icon: 'checkmark-circle', color: '#38BDF8' },
    ] satisfies Stat[];
  }, [monthSummary, monthlyData, monthlyPoints]);

  const weightHistory = useMemo(
    () => mapWeightHistoryToDisplay(weightData?.history ?? [], selectedMonth),
    [selectedMonth, weightData]
  );
  const targetWeight = useMemo(() => {
    const fallback = (weightHistory.at(-1)?.kg ?? 60) - 2.4;
    return Number((weightData?.targetWeightKg ?? fallback).toFixed(1));
      ...
      <ScrollView style={styles.container} showsVerticalScrollIndicator={false}>
        ...
        {recordError ? (
          <View style={styles.card}>
            <Text style={styles.cardTitle}>기록 데이터를 불러오지 못했어요</Text>
            <Text style={styles.cardSub}>{recordError}</Text>
          </View>
        ) : null}
